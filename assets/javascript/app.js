$(document).ready(function() {
  ...
  questions = [
    ...
    {
      q: `What is ${characters[C].name}'s homeworld?`,
      a: `${characters[C].homeworld}`,
      choices: [
        `${characters[C].films.length}`,
        `${characters[C].films.length + 2}`,
        `${characters[C].films.length - 1}`,
  ...
  function answerIncorrect() {
    $("#message").html(
      `Sorry, the correct answer is <b>${questions[Q].a}</b>.`
    );
    stop();
    runWaitTime();
  }
  ...
  function outOfTime() {
    $("#message").html(
      `Out of time... The correct answer is <b>${questions[Q].a}</b>`
    );
    stop();
    runWaitTime();
  }
  ...
  function endGame() {
    ...
    $("#timer").text("00");
    $("#buttonContainer").empty();
    $("#message").empty(); // i think this line may be unnecessary because of how .html() works
    $("#message").html(
      `You answered <b>${score}</b> correctly out of <b>${questions.length}</b>`
    );
    const backBtn = $(
      `<button id="backBtn" class="btn btn-primary btn-lg btn-block">Back</button>`
    );
